...
public class TestZeroCopyWAL {
	...
	public void testWALConfiguration() throws IOException {
		...
		EfficientWAL wal = new ZeroCopyWAL();
		Map<String, String> conf = new HashMap<>();
		conf.put(ZeroCopyWAL.WAL_DIR, walDir);
		conf.put(ZeroCopyWAL.WAL_SEGMENT_SIZE, String.valueOf(1024 * 1024 * 2));
		wal.configure(conf, es);
		File[] listFiles = new File(walDir).listFiles();
		Arrays.sort(listFiles);
		assertEquals(2, listFiles.length);
		assertEquals(ZeroCopyWAL.getSegmentFileName(walDir, 0), listFiles[1].getPath().replace("\\", "/"));
	...
	public void testWALWrites() throws IOException {
		...
		Map<String, String> conf = new HashMap<>();
		conf.put(ZeroCopyWAL.WAL_DIR, walDir);
		conf.put(ZeroCopyWAL.WAL_SEGMENT_SIZE, String.valueOf(512 * 1024));
		wal.configure(conf, es);
		int LIMIT = 1001;
		...
		wal.flush();
		RandomAccessFile raf = new RandomAccessFile(ZeroCopyWAL.getSegmentFileName(walDir, 0), "r");
		FileChannel ch = raf.getChannel();
		ch.position(0);
		System.out.println("Position:" + ZeroCopyWAL.readIntFromChannel(ch) + " " + ch.position());
		for (int i = 0; i < LIMIT; i++) {
			try {
				ByteBuffer dst = ByteBuffer.allocate(8);
				System.out.println(ZeroCopyWAL.readIntFromChannel(ch) + " pos:" + ch.position());
				ch.read(dst);
				dst.flip();
				assertEquals("test" + String.format("%04d", i), new String(dst.array()));
			} catch (Exception e) {
	...
	public void testWALReads() throws IOException {
		...
		Map<String, String> conf = new HashMap<>();
		conf.put(ZeroCopyWAL.WAL_DIR, walDir);
		conf.put(ZeroCopyWAL.WAL_SEGMENT_SIZE, String.valueOf(1024 * 1024 * 1));
		wal.configure(conf, es);
		int LIMIT = 30000;
		for (int i = 0; i < LIMIT; i++) {
	...
	public void testWALSegmentRotations() throws IOException {
		...
		Map<String, String> conf = new HashMap<>();
		conf.put(ZeroCopyWAL.WAL_DIR, walDir);
		conf.put(ZeroCopyWAL.WAL_SEGMENT_SIZE, String.valueOf(5000));
		wal.configure(conf, es);
	...
	public void testWALCommittedReads() throws IOException {
		...
		Map<String, String> conf = new HashMap<>();
		conf.put(ZeroCopyWAL.WAL_DIR, walDir);
		conf.put(ZeroCopyWAL.WAL_SEGMENT_SIZE, String.valueOf(20000));
		conf.put(WAL.WAL_ISRCHECK_DELAY, String.valueOf(10));
		conf.put(WAL.WAL_ISRCHECK_FREQUENCY, String.valueOf(10));
