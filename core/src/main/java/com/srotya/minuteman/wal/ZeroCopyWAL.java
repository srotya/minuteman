 ...
 * @author ambud
 */
public class ZeroCopyWAL implements EfficientWAL {

	private static final Logger logger = Logger.getLogger(ZeroCopyWAL.class.getName());
	private ReentrantReadWriteLock lock = new ReentrantReadWriteLock();
	private ReadLock read = lock.readLock();
	private WriteLock write = lock.writeLock();
	private FileChannel currentWrite;
	...
	@Override
	public void updateISR() throws IOException {
	...
	public void write(byte[] data, boolean flush) throws IOException {
		...
		write.lock();
		checkAndRotateSegment(dataLength + Integer.BYTES);
		try {
			counter++;
			writeIntToChannel(currentWrite, dataLength);
			currentWrite.write(ByteBuffer.wrap(data));
			writeIntToChannel(currentWrite, 0, (int) currentWrite.position());
			if (maxCounter != -1 && (flush || counter == maxCounter)) {
				flush();
				counter = 0;
			}
			logger.finest("Wrote data:" + dataLength + " at new offset:" + currentWrite.position() + " new file:"
					+ segmentCounter);
		} finally {
			write.unlock();
		}
	...
	public WALRead read(String followerId, long requestOffset, int maxBytes, boolean readCommitted) throws IOException {
		...
		int pos = readIntFromChannel(follower.getChannel(), 0);
	...
	public static int readIntFromChannel(FileChannel ch, int pos) throws IOException {
	...
	public static int readIntFromChannel(FileChannel ch) throws IOException {
		return readIntFromChannel(ch, -1);
	}

	public static void writeIntToChannel(FileChannel ch, int pos, int val) throws IOException {
		ByteBuffer dst = ByteBuffer.allocate(4);
		dst.putInt(val);
		dst.flip();
		if (pos == -1) {
	...
	public static void writeIntToChannel(FileChannel ch, int val) throws IOException {
		writeIntToChannel(ch, -1, val);
	}
	...
	private void readDataAndUpdateOffset(String followerId, long requestOffset, int maxBytes,
			...
			throws IOException {
		FileChannel buf = follower.getChannel();
	...
	private void readDataTillPos(long requestOffset, int maxBytes, WALRead readData, int segmentId, int pos,
			FileChannel channel, int offset) throws IOException {
		// int length = maxBytes < (pos - offset) ? maxBytes : (pos - offset);
		List<byte[]> data = new ArrayList<>();
		int length = 0;
		int temp = offset;
		do {
			channel.position(temp);
			int currPayloadLength = readIntFromChannel(channel);
			ByteBuffer dst = ByteBuffer.allocate(currPayloadLength);
			channel.read(dst);
			data.add(dst.array());
			length += currPayloadLength + Integer.BYTES;
			temp = offset + length;
		} while (length <= maxBytes && temp < pos);
		logger.finest("//error:" + offset + " l:" + length + " r:" + remaining(channel) + " t:" + temp + " r:"
				+ requestOffset + " s:" + segmentId + " p:" + pos + " s:" + data.size() + " b:" + channel.position());
		readData.setData(data);
	}

	public static int remaining(FileChannel ch) throws IOException {
		return (int) (ch.size() - ch.position());
	}
	...
	private WALRead initializeBuffer(MappedWALFollower follower, int segmentId, String followerId, WALRead readData)
		...
		if (!file.exists() && segmentId < segmentCounter) {
			...
			follower.setChannel(raf.getChannel());
			// skip the header
			follower.getChannel().position(Integer.BYTES);
			return null;
		}
	}
	...
	private void updateMinOffset() throws IOException {
		write.lock();
	...
	private void checkAndRotateSegment(int size) throws IOException {
		if (currentWrite == null || remaining(currentWrite) < size) {
			...
			if (forward) {
				...
				int pos = readIntFromChannel(currentWrite, 0);
				if (currentWrite.size() > getCommitOffset()) {
				...
				// new buffer
				writeIntToChannel(currentWrite, 0, 0);
			}
	...
	private long getMinimumOffset() throws IOException {
	...
	private class MappedWALFollower {
		 ...
		 *            the buf to set
		 */
		public void setChannel(FileChannel read) {
			this.read = read;
		}
